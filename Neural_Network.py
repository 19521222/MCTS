...
class CNN(nn.Module):
    def __init__(self, in_channels = 13, num_actions = 1968):
...
class NEURAL_NETWORK():
    ...
    def optimize_model(self, buffer):
        ...
        cross_entropy    = torch.sum(probability_opt * torch.nan_to_num(torch.log(act_probs)), 1)
    ...
    def save_model(self, loss): 
        if loss.item() < self.best_loss:
            ...
            self.model_cache[1] = self.optimizer.state_dict().copy()
            self.model_cache[2] = loss.item()
